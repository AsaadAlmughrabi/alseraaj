...
function FloatingPanel({anchorRef, open, onRequestClose, children, width = 760}) {
  ...
  const clear = () => { if (closeTimer.current) clearTimeout(closeTimer.current); };
  const scheduleClose = () => {
    clear();
  ...
  return createPortal(
    <div
      ...
      onPointerLeave={(e) => { if ((e.pointerType || 'mouse') === 'mouse') scheduleClose(); }}
    >
      <div
        ref={panelRef}
        className="pointer-events-auto rounded-xl border border-white/10 bg-[color:rgb(18_18_25_/95%)] backdrop-blur-xl shadow-2xl"
        style={{
          position: 'fixed',
          top: `${pos.top}px`,
          left: `${pos.left}px`,
...
export default function Navbar() {
  ...
  const announceOpen = (section) =>
    window.dispatchEvent(new CustomEvent('nav:open-section', {detail: section}));
  ...
  const ServicesPanel = () => (
    <div id={ids.servicesPanel} role="menu" aria-label={tNav('services')} className="overflow-hidden">
      <div className="px-5 py-4">
        <div className="grid md:grid-cols-2 gap-4">
          {servicesStructure.map(({groupKey, items}) => (
            <div key={groupKey} className="rounded-lg bg-white/5 p-3">
              <h3 className="mb-2 text-sm font-semibold tracking-wide text-white/80">
                ...
                {items.map((key) => (
                  <li key={key}>
                    <Link
                      ...
                      role="menuitem"
                      className="block rounded-md p-2 hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-[#FFCC00]"
                      onClick={() => setOpenSection(null)}
                    >
                      <p className="text-sm font-medium text-white/90">{tServices(`items.${key}.label`)}</p>
                      <p className="text-xs text-white/60">{tServices(`items.${key}.desc`)}</p>
                    </Link>
                  </li>
                ))}
              </ul>
  ...
  const GalleryPanel = () => (
    <div id={ids.galleryPanel} role="menu" aria-label={tNav('gallery')} className="overflow-hidden">
      <div className="px-5 py-4">
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
          {galleryKeys.map((gKey) => (
            <Link
              ...
              role="menuitem"
              className="rounded-lg bg-white/5 p-3 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-[#FFCC00]"
              onClick={() => setOpenSection(null)}
            >
              <p className="text-sm font-medium text-white/90">{tGallery(`items.${gKey}.label`)}</p>
              <p className="text-xs text-white/60">{tGallery(`items.${gKey}.desc`)}</p>
            </Link>
          ))}
        </div>
      </div>
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between py-3">
        ...
        <div className="flex items-center">
          <Link href={root} aria-label="Go to homepage" className="flex items-center">
            <Image
              src="/logos/logo.svg"
              alt="Brand logo"
              width={46}
          ...
          <ul role="menubar" className="flex items-center gap-1">
            ...
            <li role="none">
              <Link
                role="menuitem"
                href={`${root}/contact`}
                className={cx(navLink, isActive(`${root}/contact`) && 'text-white')}
              >
                {tNav('contact')}
              </Link>
            </li>
            <li role="none">
              <Link
                role="menuitem"
                href={`${root}/about`}
                className={cx(navLink, isActive(`${root}/about`) && 'text-white')}
              >
                {tNav('about')}
              </Link>
            </li>
        ...
        <div className="md:hidden">
          <button
            type="button"
            className="rounded-md p-2 text-white/90 hover:text-white focus:outline-none focus:ring-2 focus:ring-[#FFCC00]"
        ...
        <nav role="navigation" aria-label="Mobile Main">
          <ul className="space-y-2">
            <li>
              <Link
                href={root}
                className="block rounded-lg px-3 py-2 hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-[#FFCC00]"
            ...
            <li>
              <button
                type="button"
                className="w-full rounded-lg px-3 py-2 text-left hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-[#FFCC00] flex items-center justify-between"
                aria-haspopup="menu"
                ...
                <div className="ps-3">
                  {servicesStructure.map(({groupKey, items}) => (
                    <div key={groupKey} className="my-2">
                      <p className="px-3 py-1 text-xs text-white/70">
                        {tServices(`groups.${groupKey}.label`)}
                      </p>
                      <ul className="ps-2">
                        {items.map((key) => (
                          <li key={key}>
                            <Link
                              href={`${root}/services/${key}`}
                              className="block rounded-md px-3 py-2 hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-[#FFCC00]"
                              onClick={() => setMobileOpen(false)}
                            >
